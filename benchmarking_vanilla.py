...
experiment = fit(epochs = 20, lr = 0.001, model = model, train_loader = train_loader, val_loader = val_loader, bs=bs, filename='tmp_file_vani_drop.txt')


model = CIFAR10Module(minst=True,vanilla=True)
model.model.fc = DressedQuantumNet(bs, bs.nb_parameters, dropout=False)
...
# train the model with the chosen parameters
experiment = fit(epochs = 20, lr = 0.001, model = model, train_loader = train_loader, val_loader = val_loader, bs=bs, filename='tmp_file_vani_drop_no.txt')
